...
void verificaOrdem(Lista* l1){
	...
	while(i <= 4) {
		...
		if(ordem1->dados.matricula < ordem2->dados.matricula){
			printf("\nOs alunos de matricula %d e %d estao em ordem\n", ordem1->dados.matricula, ordem2->dados.matricula);
		}else{
			printf("\nOs alunos de matricula %d e %d nao estao em ordem\n", ordem1->dados.matricula, ordem2->dados.matricula);
		}
		no = no->prox;
		i++;
	}
